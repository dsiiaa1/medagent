...
**Tim:** 2 orang
**Versi dokumen:** 1.4 (update: migrasi tech stack dari Python/Streamlit ke Next.js/Supabase)
**Tanggal:** 26 Juli 2026

> **Changelog v1.3 → v1.4:** §8 — tech stack diganti total: Frontend & backend disatukan dalam Next.js (TypeScript), database berpindah dari file JSON/CSV/SQLite lokal ke Supabase (PostgreSQL), RAG berpindah dari LlamaIndex ke `pgvector` (ekstensi Supabase) dengan retrieval logic custom, orchestrator berpindah dari LangGraph (Python) ke state machine custom di Next.js Server Actions/Route Handlers, deployment berpindah dari Streamlit Community Cloud ke Vercel. §4.2, §4.3, §5, §7.2, §9, §10 disesuaikan mengikuti perubahan ini. §13 — risiko "tim belum familiar stack" dihapus dan diganti risiko baru soal maturitas ekosistem AI di Node/TypeScript. **Alasan:** rekomendasi awal (Python/Streamlit) didasarkan pada asumsi kurva belajar pendek untuk tim, namun setelah dicoba tim justru lebih mahir dan familiar dengan Next.js + Supabase — melanjutkan stack yang tidak dikuasai adalah risiko lebih besar untuk sprint 3 minggu dibanding migrasi ke stack yang sudah dikuasai tim.
>
...
> **Catatan stack (v1.4):** prototipe dibangun full-stack dalam **Next.js + Supabase** (bukan Python/Streamlit seperti rencana awal). Perubahan ini didasari kecocokan keahlian tim, bukan preferensi teknis semata — lihat §8 untuk detail dan alasan tiap komponen.
...
- Aplikasi bisa diakses lewat URL publik (Vercel) untuk keperluan juri.
...
### 4.2 Modul Orchestrator (v1.4: custom state machine di Next.js)
- Bertindak sebagai *state machine* yang mengatur urutan node: `intake` → `retrieve_context` → `urgency_scoring` → `drug_interaction_check` (paralel) → `generate_soap` → `await_doctor_verification`. Urutan dan tanggung jawab tiap node **tidak berubah** dari desain awal — yang berubah hanya implementasinya.
- **Implementasi (v1.4):** state machine ditulis manual (bukan LangGraph) sebagai kumpulan Server Actions/Route Handlers di Next.js, dengan status kasus tiap saat disimpan sebagai kolom `current_node` di tabel `cases` (Supabase). Alasan tidak memakai `@langchain/langgraph` (versi JS dari LangGraph): ekosistemnya jauh lebih baru dan kurang matang dibanding versi Python, sehingga untuk tim yang lebih menguasai Next.js/TypeScript murni, state machine custom lebih mudah dikontrol dan didebug daripada bergantung pada library yang belum stabil. Transisi antar node tetap eksplisit (bukan satu fungsi besar) agar prinsip "step terpisah, bukan satu prompt raksasa" (§4.3.1) tetap terjaga.
- Setiap transisi node dicatat sebagai baris log di tabel `case_trace` (Supabase) yang bisa ditampilkan ke UI (transparansi reasoning) — menggantikan log in-memory LangGraph.
...
- Basis pengetahuan: dokumen algoritma triase **Emergency Severity Index (ESI)**, diindeks sebagai *embeddings* di tabel Postgres dengan ekstensi **`pgvector`** (bawaan Supabase) — menggantikan LlamaIndex. Alur retrieval: dokumen dipecah jadi chunk → di-embed (mis. lewat model embedding via OpenRouter) → disimpan di kolom `vector` → saat query, similarity search (`<->` cosine distance) dijalankan langsung lewat SQL/Supabase client, hasilnya jadi konteks untuk LLM. Ini mengurangi satu dependency besar (LlamaIndex) karena vector store dan database aplikasi jadi satu sistem yang sama. ESI dipilih sebagai mesin algoritma karena Indonesia belum punya algoritma triase nasional baku yang dipublikasikan secara rinci — Permenkes No. 47/2018 mewajibkan triase berbasis keparahan klinis tapi tidak menetapkan algoritma spesifik, sehingga rumah sakit lazim mengadopsi skala internasional seperti ESI (lihat §7.3 untuk detail regulasi).
...
| b. Perhitungan skor ESI | **Kode deterministik** (TypeScript, bukan LLM) — hanya dijalankan jika `auto_scoring_eligible = true` | Terapkan decision-tree ESI resmi terhadap `patient_features.json` | `esi_score: 1-5` + alasan tiap cabang keputusan |
...
| e. Routing hasil | Orchestrator (state machine custom, §4.2) | Jika `confidence = low` **atau** `override_triggered = true` **atau** `auto_scoring_eligible = false` → tandai kasus "perlu perhatian ekstra"/"perlu penilaian manual" di dashboard (highlight visual), tetap lanjut ke SOAP tapi dengan badge peringatan | status kasus |
...
- Ini secara arsitektural direpresentasikan sebagai node `await_human_input` di orchestrator (§4.2) — alur benar-benar berhenti menunggu input dokter (status `current_node` tidak lanjut sampai ada aksi approve/edit/reject tersimpan di Supabase), bukan sekadar tombol dekoratif di UI.
...
| **Portability** | Aplikasi harus bisa dijalankan ulang dari GitHub repo tanpa setup rumit (`package.json` lengkap, `.env.example` berisi `NEXT_PUBLIC_SUPABASE_URL`/`SUPABASE_ANON_KEY`/`OPENROUTER_API_KEY`, file `schema.sql`/migration Supabase disertakan agar skema tabel + `pgvector` bisa direplikasi juri dengan satu perintah). |
...
- **(v1.4)** Disimpan sebagai baris di tabel `patients`/`cases` pada Supabase (seed script dijalankan sekali saat setup project), bukan file JSON/CSV lokal — karena database sungguhan sudah tersedia gratis lewat Supabase, tidak ada alasan lagi memakai file statis; ini juga membuat dashboard Realtime (§9) bisa langsung bekerja terhadap data yang sama.
...
## 8. Arsitektur Teknis & Tech Stack (v1.4)

| Layer | Teknologi | Alasan |
|---|---|---|
| **Frontend/UI** | Next.js (App Router, React, TypeScript) | Tim lebih mahir dan berpengalaman di stack ini dibanding Python/Streamlit (lihat proyek SIBIMKON sebelumnya) — kurva belajar hilang, bukan cuma pendek. Server Components untuk halaman statis, Client Components untuk form interaktif (§9). |
| **Backend/API** | Next.js Route Handlers & Server Actions | Menyatukan frontend & backend dalam satu framework — lebih sederhana dari split Streamlit (UI) + Python backend terpisah yang direncanakan sebelumnya. |
| **Orchestrator (state machine)** | Custom TypeScript, bukan LangGraph.js | `@langchain/langgraph` versi JS jauh lebih baru/kurang matang dibanding versi Python. Untuk tim yang menguasai TypeScript murni, state machine manual (node eksplisit + status disimpan di kolom `current_node` tabel `cases`) lebih mudah dikontrol dan didebug — lihat §4.2. |
| **RAG & Knowledge Base** | Supabase `pgvector` + retrieval logic custom | Menggantikan LlamaIndex. Dokumen ESI di-chunk → di-embed → disimpan sebagai vector di Postgres → similarity search lewat SQL langsung. Satu sistem untuk vector store + database aplikasi, bukan dua tool terpisah — lihat §4.3. **Ini satu-satunya komponen yang benar-benar baru bagi tim; disarankan spike 1-2 hari di awal sprint untuk validasi (lihat §11).** |
| **LLM Gateway** | OpenRouter | Satu API key untuk akses banyak model (Gemini, Mistral, Llama via Groq, GPT, Claude, dll) — memudahkan model-agnostic routing dan jadi fallback kalau satu provider down/rate-limited saat live demo. Pola fallback-antar-provider ini sudah pernah diimplementasikan tim di SIBIMKON (`generateWithFallback`), tinggal diterapkan ulang. |
...
| **Bahasa Utama** | TypeScript | Satu bahasa untuk seluruh stack (Next.js frontend + backend + Supabase client) — tim sudah berpengalaman, type-safety juga membantu validasi tipe data klinis (`age_months: number`, `esi_score: 1\|2\|3\|4\|5`, dll). |
| **Penyimpanan Data** | Supabase (PostgreSQL) | Menggantikan file JSON/CSV/SQLite lokal. Database sungguhan tersedia gratis, mendukung Realtime subscription (dashboard auto-update, lihat §9), dan `pgvector` untuk RAG di sistem yang sama. Auth bawaan juga mempermudah roadmap multi-role (§14). |
| **Deployment** | Vercel | Gratis, auto-deploy dari GitHub, menghasilkan URL publik untuk juri — setara kemudahannya dengan Streamlit Community Cloud, native untuk Next.js. |
...
## 9. Struktur Halaman UI (Next.js)

1. **Halaman Input** (`/input`) — form intake pasien baru (perawat).
2. **Halaman Dashboard** (`/dashboard`) — daftar pasien terurut prioritas, color-coded ESI 1-5. **(v1.4)** memakai Supabase Realtime subscription agar daftar auto-update saat ada pasien baru/status berubah, tanpa refresh manual — kemampuan tambahan yang tidak dimiliki rencana Streamlit sebelumnya.
3. **Halaman Detail Kasus** (`/case/[id]`) — SOAP summary, referensi sumber, hasil cek interaksi obat, reasoning trace ringkas (dari tabel `case_trace`).
4. **Halaman Verifikasi** (terintegrasi di `/case/[id]` atau `/verifikasi`) — tombol approve/edit/reject, riwayat siapa yang memverifikasi.
...
- **Orang A — Agent & Backend:** state machine orchestrator (§4.2), retrieval RAG dengan `pgvector` (§4.3), Agent Farmasi, prompt engineering, guardrail validasi output, skema Supabase.
- **Orang B — UI & Data:** dashboard Next.js, form input, penyusunan dataset sintetis (§7.2), integrasi hasil dari backend ke tampilan, Realtime subscription, dokumentasi.

Titik integrasi (sebaiknya disepakati sejak awal): skema tabel Supabase (`cases`, `patients`, `case_trace`, dll.) yang jadi kontrak antara logic backend dan komponen frontend — bukan lagi format JSON lepas seperti rencana Streamlit sebelumnya, karena sekarang keduanya berbagi database yang sama secara langsung.
...
| **Hackathon Sprint** | 13 Agu – 3 Sep 2026 | Implementasi PRD ini: **hari 1-2 minggu 1 = spike validasi `pgvector`** (embed 1 dokumen ESI, similarity search, pastikan hasil masuk akal — lihat §8), lanjut minggu 1 = P0 (orchestrator + RAG dasar berjalan), minggu 2 = P1 (agent farmasi + dashboard lengkap) + integrasi, minggu 3 = P2 (mock fitur tambahan) + polish UI + dokumentasi + video demo. |
...
|---|---|---|
| AI Agent Implementation | 30% | Orchestrator state machine nyata (§4.2) + tool calling aktif ke RAG & drug-check, bukan prompt tunggal. |
...
| **(v1.4) `pgvector`/embedding pipeline adalah komponen baru bagi tim** — satu-satunya bagian stack yang belum pernah diimplementasikan sebelumnya | Spike validasi 1-2 hari di awal sprint (§11) sebelum komit ke seluruh arsitektur RAG; kalau gagal, fallback ke retrieval sederhana (keyword search di Postgres) sambil tetap menjaga prinsip "LLM tidak menentukan skor sendirian" (§4.3.1). |
